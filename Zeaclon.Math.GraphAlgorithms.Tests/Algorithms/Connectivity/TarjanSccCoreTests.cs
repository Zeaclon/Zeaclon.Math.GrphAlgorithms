...
namespace TestProject1.Algorithms.Connectivity
{
    public class TarjanSCCTests
    {
        ...
        public void SingleNodeGraph_ShouldReturnOneSCC()
        {
            ...
            var sccs = TarjanSCC.FindStronglyConnectedComponents(graph);
        ...
        [Fact]
        public void ComplexGraph_ShouldIdentifyCorrectSCCs()
        {
            // Graph: A->B, B->C, C->A, B->D, D->E, E->D
            ...
            Assert.Equal(2, sccs.Count); // Two SCCs: {A,B,C} and {D,E}
            Assert.Contains(sccs, scc => scc.Contains(A) && scc.Contains(B) && scc.Contains(C));
            Assert.Contains(sccs, scc => scc.Contains(D) && scc.Contains(E));
        }
    }   
}